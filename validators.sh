...
curl $CLRPCADDRESS:$CLRPCPORT/eth/v1/beacon/states/head/validators > $VALIDATORSFILE

echo "Extracting relevant validator data into CSV..."
jq -r ' (["vindex","balance","status","pubkey","withdrawal_credentials"]), (.data[] | [ .index, .balance, .status, .validator.pubkey, .validator.withdrawal_credentials ]) | @csv ' $VALIDATORSFILE > $VALIDATORSTEMPFILE
